...
func main() {
	...
	store, err := firefox.NewStore("https://addons.mozilla.org/")
	if err != nil {
		log.Fatalf("Error creating store: %v", err)
	}

	err = store.Update(client, "tmp/extension.zip", "tmp/source.zip")
	if err != nil {
		log.Fatalf("Error uploading source: %v", err)
	}

	log.Println("Successfully uploaded new extension")
}
